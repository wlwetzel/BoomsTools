...
if __name__ == '__main__':
    ...
    subdirs = ['magnetometer', 'imager_data', 'gps', 'misc', 'spectrometer_data', 'imager_event_stats', 'imager_housekeeping', 'pps']
    for subdir in subdirs:
        full_path = os.path.join(baseFolder, subdir)
        if not os.path.exists(full_path):
            os.makedirs(full_path)
    make_plaintext(sys.argv[1])
